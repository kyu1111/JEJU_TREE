              ...
              success: function(data) {
                  if (data && data.response && data.response.body && data.response.body.items && data.response.body.items.item.length > 0) {
                      ...
                      var content = "<form id='markerDataForm' action='<%=request.getContextPath()%>/plans_insert_ok.go' method='post'>"
                          ...
                          + "<div style='padding:10px; width:250px;'>" + item.title + "&nbsp;&nbsp;<button id='selectBtn' type='submit'>Select</button>&nbsp;&nbsp;<button id='closeBtn'>Close</button>" +  "</div>"
                          + "<div>" + item.addr1 + "</div>"
                          + "<div><img src='" + item.firstimage2 + "' alt='Image' style='width:100px; height:auto;'></div>"
                          + "<p>Start Date : <input type='date' name='start_date'></p>"  // Start Date input field
                          + "<p>End Date : <input type='date' name='end_date'></p>"  // End Date input field
                          + "</form>";
...
<br>
	<div>
		<input type="text" class="text_custom" id="searchKeyword" name="searchKeyword" value="장소를 키워드로 검색하세요.">	
