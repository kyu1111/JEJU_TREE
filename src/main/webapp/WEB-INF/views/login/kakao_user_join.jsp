...
<div id = "container">
		...
		<form enctype="multipart/form-data" method="post" name="joinForm" id="joinForm" action="<%=request.getContextPath()%>/kakao_join_ok.go">
		</c:if>
		<c:if test="${!empty share_id }">
		<form enctype="multipart/form-data" method="post" name="joinForm" id="joinForm" action="<%=request.getContextPath()%>/invitedKakao_join_ok.go">
		<!--초대 페이지 일시 초대자 아이디 같이 넘겨 주기  -->
		<input type="hidden" name ="share_id" value = "${share_id}">
		</c:if>
			<div class = "form_field">
				<input readonly type="text" name = "user_id" id = "user_id" class="id_check" value = "${param.user_email}">
				<span class = "id_check"></span>
			</div>
			<div class = "form_field">
				<input type="password" name="user_pwd" id="user_pwd" placeholder="비밀번호: " autocomplete="off">
			</div>
			<div class = "form_field">
				<input type="password" name="user_repwd" id="user_repwd" placeholder="비밀번호 확인: " autocomplete="off">
			</div>
			<div class = "form_field">
			...
			<div class = "form_field">
				<input type="text" name="user_phone" id="user_phone" placeholder="h.p: ">
			</div>
			<div class = "form_field">
				<input type="text" readonly name="user_email" id="user_email" class="email_check" value = "${param.user_email}">
			</div>
